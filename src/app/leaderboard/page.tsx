...
function LeaderboardContent() {
  ...
  const hasMore = data?.traders.length === ITEMS_PER_PAGE;
      ...
      <main className="relative z-10 flex-1 px-3 sm:px-4 md:px-6 lg:px-8 pt-6 pb-8">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 rounded-2xl bg-gradient-to-br from-yellow-500 to-amber-500 shadow-lg shadow-yellow-500/25">
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            {data && (
              <span>
                Showing {offset + 1}-{offset + data.traders.length} traders
              </span>
            )}
          </div>
        </motion.div>
        ...
        {/* Pagination */}
        {!isLoading && data && data.traders.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
