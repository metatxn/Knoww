...
test.each(["feed", "stream"])(
  "%s startup connects panel settings and preserves discovery order",
  async (surface) => {
    const order: string[] = [];
    ...
    Object.assign(window, {
      ...
      KNOWW_UI: {
        ...
        setNotificationStackVisibility: vi.fn(),
      },
      KNOWW_UTILS: {
        log: vi.fn(),
        safeSendMessage,
    ...
    await import("../../src/content/main");
    await vi.waitFor(() =>
      expect(window.KNOWW_CONFIG.onSettingsChange).toHaveBeenCalledTimes(1)
    );
    if (surface === "feed") {
      ...
      expect(watchFeed).not.toHaveBeenCalled();
    }
    const changed = vi.mocked(window.KNOWW_CONFIG.onSettingsChange).mock
      .calls[0][0];
    const settings = {
      platforms: { twitter: true },
      showNotificationStack: true,
      notificationPanelSurface: "floating",
    } as Parameters<typeof changed>[0];
    changed(settings);
    expect(
      window.KNOWW_UI.setNotificationStackVisibility
    ).toHaveBeenLastCalledWith(true);
    changed(settings);
    expect(
      window.KNOWW_UI.setNotificationStackVisibility
    ).toHaveBeenCalledTimes(1);
    changed({ ...settings, showNotificationStack: false });
    expect(
      window.KNOWW_UI.setNotificationStackVisibility
    ).toHaveBeenLastCalledWith(false);
    changed({ ...settings, notificationPanelSurface: "sidebar" });
    expect(
      window.KNOWW_UI.setNotificationStackVisibility
