...
function projectPosition(
  row: Awaited<ReturnType<PolymarketClient["fetchWalletPositions"]>>[number]
) {
  return {
        ...
        (args, context) =>
          executePublicRead(
            ...
            async () => {
              ...
              const row = await client.fetchPublicProfile(args.walletAddress, {
  ...
  registerWithLegacyAlias(
    ...
    (name, description) =>
      server.registerTool(
        ...
        (args, context) =>
          executePublicRead("polymarket_get_wallet_pnl", context, async () => {
            const client = requirePolymarketClient();
            const [rows, allTimeRow] = await Promise.all([
              ...
              client.fetchWalletAllTimePnl(args.walletAddress, {
                signal: context.mcpReq.signal,
              }),
            ]);
            ...
            return {
              ...
              structuredContent: { pnl, meta: dataMeta(client, { truncated }) },
            };
          })
      )
  );
      ...
      server.registerTool(
        ...
        (args, context) =>
          executePublicRead(
            ...
            async () => {
              const client = requirePolymarketClient();
              const portfolio = await client.fetchWalletPortfolioValue(
                args.walletAddress,
                {
                  signal: context.mcpReq.signal,
                }
              ...
              return {
                ...
                structuredContent: { portfolio, meta: dataMeta(client) },
              };
            }
          )
      )
