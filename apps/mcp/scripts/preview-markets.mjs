...
<h1>Knoww MCP Apps fixture preview</h1><p>Sample data only. This host simulates the MCP Apps bridge.</p>
<nav><button data-mode="normal">Markets</button><button data-mode="empty">Empty</button><button data-mode="partial">Partial failure</button><button data-mode="error">Tool error</button><button data-mode="hostile">Untrusted text</button><button id="theme">Toggle theme</button></nav>
...
const slugs = ["fed-rate-cut", "bitcoin-year-end"];
function result() {
  if(mode === "error") return {isError:true,content:[{type:"text",text:"UPSTREAM_UNAVAILABLE"}]};
  return {structuredContent:{selectionSlugs: mode === "empty" ? [] : slugs,markets:mode === "empty" ? [] : [
    {id:"1",slug:slugs[0],question:mode === "hostile" ? '<img src=x onerror="alert(1)"> Will rates fall?' : "Will the Fed cut rates at its next meeting?",outcomes:[{name:"Yes",price:"0.625",priceLabel:"62.5%",tokenId:"111"},{name:"No",price:"0.375",priceLabel:"37.5%",tokenId:"222"}],description:"Resolves based on the Federal Reserve's announced target rate following its next scheduled meeting.",url:mode === "hostile" ? "javascript:alert(1)" : "https://knoww.app/events/detail/fed-rate-cut",endDate:"2026-12-31T00:00:00Z",volume:"1248500.25"},
  ...
  ],omittedCount:0,unavailableCount:mode === "partial" ? 1 : 0,meta:{asOf:new Date().toISOString()}}};
}
...
createServer((request, response) => {
  ...
  response.end(request.url === "/widget" ? MARKETS_HTML : preview);
