...
Edit `apps/mcp/server.json` to change the listing. Its version is the baseline for automatic patch increments. CI reads every page of registry history, including deleted versions, and finds the highest stable `MAJOR.MINOR.PATCH` version or the checked-in baseline, whichever is greater. It increments only the patch number: `0.2.1` becomes `0.2.2`, then `0.2.3`. Prerelease and calendar CI versions do not determine the next patch number. A higher checked-in major or minor baseline starts increments from that baseline. Routine publication does not need a version bump or a bot commit.

CI writes the generated manifest to the runner's temporary directory. Publisher-provided metadata records the GitHub run ID and commit SHA. Retrying a published run reuses that version and skips publication when its metadata and active status still match, even if newer versions have since been published. Conflicting metadata, inactive versions, and incomplete registry history fail the job. Workflow concurrency serializes publications from this workflow; a conflicting manual publication fails rather than overwriting an existing version. Use **Run workflow** for a new patch release or rerun an existing workflow to recover its publication.

On September 22, 2026, the accidental `2026.9.22-ci.35685132784` entry was marked `deleted` after publishing `0.2.2`. The registry then confirmed `0.2.2` as active and latest. Merely deprecating the calendar version would have left it eligible as the highest version. For a similar recovery, publish the corrected stable release first, then mark only the mistaken version `deleted` with `mcp-publisher status`. This retires the registry entry without removing the MCP service, and the registry recalculates its latest version.
