 ...
 * `src/markets/search.ts` verbatim; the only differences are that the base
 * URL comes from the client context and the fetchers are built per client.
 */
...
export function createGammaSearch(ctx: PolymarketClientContext) {
  ...
  async function fetchPublicSearchEvents(
    ...
    const payload = await fetchJsonFromGamma(
      `${ctx.baseUrls.gamma}/public-search?${params.toString()}`,
      options
    );
  ...
  async function fetchTagEvents(
    ...
    const payload = await fetchJsonFromGamma(
      `${ctx.baseUrls.gamma}/events/keyset?${params.toString()}`,
      options
    );
  ...
  return { fetchPublicSearchEvents, fetchTagEvents, fetchAggregatedSearchData };
}

export type PolymarketGammaSearch = ReturnType<typeof createGammaSearch>;
