 ...
 *
 * Behaviour is the legacy `src/markets/orderbook.ts` verbatim; only the base
 * URL and fetch binding come from the context.
 */
...
export function createClobOrderbook(ctx: PolymarketClientContext) {
  /**
   ...
   */
  async function fetchOrderbookByTokenId(
    tokenId: string,
    ...
    return withUpstreamTimeout(
      ...
      async (fetchImpl, signal) => {
        ...
        const response = await fetchImpl(
          `${ctx.baseUrls.clob}/book?${params.toString()}`,
          {
  ...
  return { fetchOrderbookByTokenId };
}

export type PolymarketClobOrderbook = ReturnType<typeof createClobOrderbook>;
